...
namespace Midif.V3 {
	public struct MidiSequencer {
		public Ref<MidiFile> file;
		public Ref<MidiSynth> synth;
		...
		public MidiSequencer(Ref<MidiFile> file, Ref<MidiSynth> synth) {
			this.file = file;
			this.synth = synth;

			int trackCount = file.value.trackCount;
		...
		public void Reset() {
			for (int i = 0, count = file.value.trackCount; i < count; i += 1) {
				trackIndices[i] = 0;
				trackTicks[i] = 0;
			}
		...
		public void AdvanceTime(double time) {
			ticks += time * beatsPerSecond * file.value.ticksPerBeat;

			isFinished = true;
			for (int i = 0, count = file.value.trackCount; i < count; i += 1) {
				int trackLength = file.value.trackLengths[i];
				if (trackIndices[i] >= trackLength) continue;
				isFinished = false;

				MidiEvent[] track = file.value.tracks[i];
		...
		public void HandleEvent(int track, MidiEvent e) {
			if (e.status == 0xff && e.type == 0x51) {  // meta tempo
				...
				int microsecondsPerBeat = BitBe.ReadInt24(file.value.bytes, ref i);
			...
			case 0x8:  // note off
				...
				synth.value.NoteOff(track, channel, e.b1, e.b2);
				break;
			case 0x9:  // note on
				Debug.LogFormat("note on: {0} {1} {2} {3}", track, channel, e.b1, e.b2);
				synth.value.NoteOn(track, channel, e.b1, e.b2);
				break;
			case 0xa:  // aftertouch
			...
			case 0xb:  // controller
				...
				synth.value.Controller(track, channel, e.b1, e.b2);
				break;
			...
			case 0xe:  // pitch bend
				...
				synth.value.PitchBend(track, channel, e.b1, e.b2);
				break;
			default:

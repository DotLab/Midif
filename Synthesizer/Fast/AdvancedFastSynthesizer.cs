	...
	public class AdvancedFastSynthesizer : MidiSynthesizer {
		...
		public class Voice {
			...
			public void SetGain (double channelGain, double channelPanLeft, double channelPanRight) {
				Gain = MidiTables.Recip127[Velocity] * channelGain;
		...
		// Semitones in both direction.
		const double PitchBendRange = 0;
...
//				oldest.Gain = gainTable[velocity, channels[channel].Volumn, channels[channel].Expression];
		...
		public override void Controller (byte channel, MidiControllerType controller, byte value) {
			...
			case MidiControllerType.Pan:
				...
				if (value > 64) {
					...
					channels[channel].PanRight = MidiTables.Recip127[value - 64] * 2;
				} else if (value < 64) {
					channels[channel].PanLeft = MidiTables.Recip127[64 - value] * 2;
		...
		public override void RenderStereo (ref double sampleL, ref double sampleR) {
			foreach (var voice in voices)
				if (voice.Note != 0) {

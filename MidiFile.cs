...
namespace Midif {
	public class MidiFile {
		...
		public MidiFile (Stream midiStream) {
		...
		void DecodeMidiFromStream (Stream stream) {
			...
			midiHeader.SetTimeFormat(division);
...
//						}
					...
					case 0xFF:
						...
						case 0x01:
							...
							midiEvent.Parameters[1] = Encoding.UTF8.GetString(tmp, index, ((int)tmp[index - 1])); index += (int)tmp[index - 1];
							break;
						case 0x02:
						...
						case 0x04:
							...
							midiEvent.Parameters[0] = Encoding.UTF8.GetString(tmp, index + 1, (int)tmp[index]);
		...
		static int GetChannel (byte statusbyte) {
			statusbyte = (byte)(statusbyte << 4);
			return statusbyte >> 4;
		}

		static uint GetDeltaTime (byte[] data, int currentIndex, ref UInt16 numOfBytes) {
			int value = 0;
			byte b;
